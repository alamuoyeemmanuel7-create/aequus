...
const ChartContent = dynamic(
  async () => {
    ...
    return {
      default: ({ data, isRateLimiter }: { data: any[]; isRateLimiter: boolean }) => (
...
function FeeChartInner({ sim }: FeeChartProps) {
  const isRateLimiter = sim.baseFee.mode === 'rate-limiter';
  
  const data = useMemo(() => 
    simulateFeeCurve(sim).map((p) => ({
      x: p.x,
      feeBps: p.feeBps,
  ...
  return (
    <div className="h-64 w-full">
      <Suspense fallback={<div className="h-64 w-full bg-white/5 rounded animate-pulse" />}>
        <ChartContent data={data} isRateLimiter={isRateLimiter} />
      </Suspense>
    </div>
  );
}
